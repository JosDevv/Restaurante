...
function crearTabla() {
    if (objDatos.filter==="") {
        ...
        objDatos.recordsFilter=objDatos.records.filter(item=>{
            ...
            if ((nombres.toLowerCase().search(objDatos.filter.toLowerCase())!=-1) || (apellidos.toLowerCase().search(objDatos.filter.toLowerCase())!=-1) || (usuario.toLowerCase().search(objDatos.filter.toLowerCase())!=-1) || (ntipo.toLowerCase().search(objDatos.filter.toLowerCase())!=-1)) {
                return item;
    ...
    objDatos.recordsFilter.forEach((item,index) => {
        ...
        <td scope="col"><button class="btn btn-primary btn-xs" onClick="editarUsuario(${item.idusuario})" ><i class="far fa-edit"></i></button>
        <button class="btn btn-danger btn-xs" onClick="eliminarUsuario(${item.idusuario})" ><i class="far fa-trash-alt"></i></button></td>
        </tr>
